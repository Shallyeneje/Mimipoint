...
export default function AirtimeDashboard() {
  ...
  const [selectedPrice, setSelectedPrice] = useState(""); // Store price for confirmation modal
  const [phoneNumber, setPhoneNumber] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [confirmationModal, setConfirmationModal] = useState(false);
  const [amount, setAmount] = useState(""); // Store the selected amount

  const handleCardClick = (price: string) => {
    ...
    <div className="min-h-screen bg-[#EFEFF5] p-8">
      ...
      <div className="grid md:grid-cols-4 gap-4 mt-6">
        {data.map(({ name, description, icon: Icon, color, nameColor, bg }) => (
          <Card
            key={name}
            className="px-3 py-2.5 h-[107px] flex items-center cursor-pointer rounded-[6px]"
      ...
      <div className="flex">
        <main className="flex-1">
          ...
          >
            <div className="flex flex-col">
              ...
              <input
                ...
                value={amount}
                onChange={(e) => setAmount(e.target.value)} // Allows user to modify amount
              />
          ...
          <DialogModal
            ...
            showFooter={false}
          >
            <h2 className="text-blue-900 text-3xl font-bold">₦{amount}</h2>
            ...
            <div className="bg-gray-200 p-2 rounded-md mt-4 flex justify-between">
